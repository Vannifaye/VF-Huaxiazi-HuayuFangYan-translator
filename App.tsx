...
const ATLAS_DATA: AtlasItem[] = [
  {
    name: '粤语',
    region: '两广及港澳',
    description: '保留了大量古汉语特征，有九声六调，极具音乐感和韵律美。',
    classicPhrase: '雷猴',
    classicMeaning: '你好',
    features: ['九声六调', '入声保留', '词汇古雅'],
    history: '源于中原雅言，在岭南地区融合了百越语言特征而成。'
  },
  {
    name: '吴语',
    region: '苏沪浙',
    description: '被称为“吴侬软语”，语调平稳委婉，清淡优雅。',
    classicPhrase: '霞霞侬',
    classicMeaning: '谢谢你',
    features: ['全浊音保留', '软糯平缓', '词汇考究'],
    history: '上古时期越人语言与中原汉语多次融合的产物。'
  },
  {
    name: '闽南语',
    region: '闽南及台湾',
    description: '中国最古老的方言之一，被称为“古汉语的活化石”。',
    classicPhrase: '哩厚',
    classicMeaning: '你好',
    features: ['十五音系统', '文白异读', '古味浓厚'],
    history: '西晋永嘉之乱时期，中原百姓南迁带来的河洛话演变而来。'
  },
  {
    name: '西南官话',
    region: '川渝滇黔',
    description: '幽默风趣，富有节奏感，生活化气息极强。',
    classicPhrase: '抓子嘛',
    classicMeaning: '在干什么',
    features: ['儿化韵', '平仄混合', '俚语丰富'],
    history: '明清时期“湖广填四川”大移民运动形成的复合型方言。'
  }
];
...
const App: React.FC = () => {
  ...
  const [selectedDialect, setSelectedDialect] = useState<Dialect>(Dialect.CANTONESE);
  ...
  useEffect(() => {
    const savedHistory = localStorage.getItem('huaxiazi_v5_history');
    if (savedHistory) setHistory(JSON.parse(savedHistory));
    
    const savedProfile = localStorage.getItem('huaxiazi_v5_profile');
  ...
  const saveToHistory = (res: TranslationResult, original: string) => {
    ...
    setHistory(updated);
    localStorage.setItem('huaxiazi_v5_history', JSON.stringify(updated));
  };
      ...
      <div className="bg-white rounded-[1.8rem] shadow-xl shadow-red-900/5 border border-slate-50 overflow-hidden">
        <div className="px-5 py-4 bg-red-50/10 flex items-center justify-between border-b border-red-50/20">
          ...
          >
            {Object.entries(DIALECT_CATEGORIES).map(([cat, ds]) => (
              <optgroup label={cat} key={cat} className="text-slate-300">
                {ds.map(d => <option key={d} value={d} className="text-red-700">{d.split(' ')[0]}</option>)}
              </optgroup>
            ))}
          </select>
        </div>
        ...
        <div className="p-6">
          <textarea
            ...
            placeholder={isRecording ? "正在采集乡音..." : "输入普通话或长按右侧识别..."}
          ...
          <div className="relative w-full max-w-sm bg-white rounded-[2rem] shadow-2xl overflow-hidden animate-in zoom-in-95 slide-in-from-bottom-4 duration-500">
            ...
            <div className="p-6 space-y-4">
              ...
              <section className="bg-red-50/20 p-4 rounded-xl border border-red-50">
                <p className="text-[8px] font-black text-red-400 uppercase mb-2 tracking-widest">经典词汇</p>
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-lg font-black text-red-600">{selectedAtlas.classicPhrase}</p>
                    <p className="text-[9px] text-slate-400 font-bold italic">意为：{selectedAtlas.classicMeaning}</p>
                  </div>
              ...
              <button onClick={() => setSelectedAtlas(null)} className="w-full py-3 mt-2 bg-slate-900 text-white text-[10px] font-black uppercase rounded-xl active:scale-95 transition-all">关闭预览</button>
            </div>
          </div>
        </div>
      )}
  ...
  const renderHistoryTab = () => (
    <div className="space-y-3 animate-in slide-in-from-right-4 duration-500 pb-36">
      <div className="flex items-center justify-between px-2 mb-2">
        ...
        <button onClick={() => { setHistory([]); localStorage.removeItem('huaxiazi_v5_history'); }} className="text-[8px] font-black text-slate-300 hover:text-red-600 transition-colors">清空足迹</button>
      </div>
      {history.length === 0 ? (
    ...
    <div className="space-y-4 animate-in slide-in-from-bottom-6 duration-700 pb-36">
          ...
          <div key={idx} className="px-6 py-4 flex flex-col hover:bg-red-50/5 transition-all">
            ...
            <input 
              ...
              onChange={e => {
                ...
                localStorage.setItem('huaxiazi_v5_profile', JSON.stringify(updated));
              }}
            />
          </div>
        ))}
      ...
      <div className="bg-slate-900 rounded-[1.5rem] p-5 text-white flex justify-between items-center group cursor-pointer shadow-lg active:scale-[0.98] transition-all">
          <div className="flex items-center space-x-3">
            ...
            <p className="text-[9px] font-black uppercase tracking-widest">使用帮助与反馈</p>
          </div>
